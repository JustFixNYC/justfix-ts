...
- Support for React 15 dropped.
- All dependencies (focus-trap-react, focus-trap, no-scroll, react-displace, and tabbable) are now included with the package directly. They may be split out into separate modules again in the future, but for now bundling them into one package was the most expedient way to fork and iterate.
- TypeScript definitions are now included in the package by default, since the project's source code was converted to TypeScript.
